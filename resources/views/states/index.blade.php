				...
				@foreach ($states as $state)
					...
					<tr>
						...
						<td>
							...
							<form action="{{ route('states.archived', $state->id) }}" method="POST">
								{{ csrf_field() }}
								{{ method_field('PUT') }}
							...
							<form action="{{ route('states.delete', $state->id) }}" method="POST">
								{{ csrf_field() }}
								{{ method_field('DELETE') }}

								<a href="{{ route('states.edit', $state->id) }}" class="btn btn-link">
